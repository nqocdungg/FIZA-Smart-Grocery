...
public interface ShoppingListItemRepository extends JpaRepository<ShoppingListItem, Long> {
        ...
        @Query(value = """
                        SELECT
                            combined.food_id AS id,
                            combined.foodName AS foodName,
                            combined.unit AS unit,
                            SUM(combined.count) as total_count
                        FROM (
                            SELECT
                                sli.food_id,
                                COALESCE(sli.custom_name, f.name) AS foodName,
                                COALESCE(sli.unit, f.unit) AS unit,
                                COUNT(sli.id) as count
                            FROM shopping_list_items sli
                            LEFT JOIN foods f ON sli.food_id = f.id
                            JOIN shopping_lists sl ON sli.shopping_list_id = sl.id
                            WHERE sl.family_id = :familyId
                            GROUP BY sli.food_id, sli.custom_name, f.name, sli.unit, f.unit

                            UNION ALL

                            SELECT
                                fi.food_id,
                                COALESCE(fi.custom_name, f.name) AS foodName,
                            ...
                            WHERE fi.family_id = :familyId
                              ...
                              AND fi.quantity > 0
                              AND (
                                  (LOWER(TRIM(COALESCE(f.unit, 'kg'))) = 'g' AND fi.quantity <= 100) OR
                                  (LOWER(TRIM(COALESCE(f.unit, 'kg'))) = 'kg' AND fi.quantity <= 0.1) OR
                                  (LOWER(TRIM(COALESCE(f.unit, 'kg'))) = 'ml' AND fi.quantity <= 200) OR
                                  (LOWER(TRIM(COALESCE(f.unit, 'kg'))) IN ('l', 'lit', 'lít') AND fi.quantity <= 0.2) OR
                                  (LOWER(TRIM(COALESCE(f.unit, 'kg'))) = 'quả' AND fi.quantity <= 2) OR
                                  (LOWER(TRIM(COALESCE(f.unit, 'kg'))) NOT IN ('g', 'kg', 'ml', 'l', 'lit', 'lít', 'quả') AND fi.quantity <= 1)
                              )
                        ) combined
                        GROUP BY combined.food_id, combined.foodName, combined.unit
